...
export default function SectionPage() {
  ...
  const findLessonAndSection = (course: any, lessonSlug: string, sectionSlug: string) => {
  ...
  useEffect(() => {
    const loadSectionContent = async () => {
      ...
      const { lesson, section } = findLessonAndSection(course, lessonSlug, sectionSlug);
      
      if (lesson && section) {
        setIsLoadingContent(true);
        try {
          const content = await getSectionContent(lesson.lesson, section);
    ...
    return (
      <div className="p-8">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          <h2 className="text-yellow-800 font-semibold mb-2">Section Not Found</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <nav className="mb-8 text-sm text-gray-600">
        <span className="font-medium text-blue-600">{course.learningPlan.title}</span>
      ...
      {isLoadingContent ? (
        <div className="flex items-center justify-center h-64">
          <div className="text-lg text-gray-600">Loading section content...</div>
        </div>
      ) : sectionContent ? (
              ...
              <div className="space-y-4">
                {sectionContent.examples.map((example, index) => (
                  <div key={index} className="bg-white rounded-lg border border-blue-200 p-4">
                    <p className="text-blue-800 leading-relaxed">{example}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                {sectionContent.exercises.map((exercise, index) => (
                  <div key={index} className="bg-white rounded-lg border border-purple-200 p-4">
                    <div className="flex items-start gap-3">
                      <span className="bg-purple-100 text-purple-800 text-sm font-medium px-2 py-1 rounded mt-1">
                        {index + 1}
                      </span>
                      <p className="text-purple-800 leading-relaxed flex-1">{exercise}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600">
            This section's content hasn't been generated yet. Please check back later.
          </p>
        </div>
      )}
    </div>
